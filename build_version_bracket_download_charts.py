...
OUTPUTS = {
    "suv": OUT / "suv-version-market-share-30-50-2025-2026.png",
    "passenger": OUT / "passenger-version-market-share-25-50-2025-2026.png",
}
PAGE = OUT / "download-version-bracket-market-share-pngs.html"
...
def compute(path, year, segment, low, high, top_n=8):
    df = pd.read_excel(path, sheet_name="Export", usecols=[0, 2, 3, 5, 6, 7, 10])
    df.columns = ["year", "brand", "model", "version", "price", "q", "segment"]
    df["year"] = pd.to_numeric(df["year"], errors="coerce")
    df["brand"] = df["brand"].map(clean)
    df["model"] = df["model"].map(map_model)
    df["version"] = df["version"].map(clean)
    ...
    grouped = (
        filtered.groupby(["brand", "model", "version"], as_index=False)
        .agg(units=("q", "sum"))
        .query("units > 0")
        .sort_values("units", ascending=False)
    )
    grouped["label"] = grouped["brand"] + " " + grouped["model"] + " | " + grouped["version"]
...
def draw_panel(draw, y0, period, rows, max_share, chart):
    ...
    text(draw, (label_x, top - 16), "Marca / modelo / versión", fill=MUTED, font_obj=SMALL)
    ...
    for idx, (label, units, share) in enumerate(rows):
        y = top + idx * row_h + 9
        display = label if len(label) <= 72 else label[:71] + "..."
        text(draw, (label_x, y + 4), display, font_obj=LABEL)
...
def make_download_page():
    ...
    html = f"""<div id="download-version-bracket-market-share" class="download-version-bracket-market-share">
  <h2>Descargar gráficos PNG por versión</h2>
  <div class="download-grid">
    <section>
      <button class="btn btn-primary" type="button" data-file="suv">Descargar PNG SUV</button>
      <img alt="Vista previa SUV por versión" src="data:image/png;base64,{suv_encoded}">
    </section>
    <section>
      <button class="btn btn-primary" type="button" data-file="passenger">Descargar PNG Vehículos de Pasajeros</button>
      <img alt="Vista previa Vehículos de Pasajeros por versión" src="data:image/png;base64,{passenger_encoded}">
    </section>
  </div>
</div>

<style>
  #download-version-bracket-market-share {{
    width: 100%;
    color: var(--foreground);
  }}

  #download-version-bracket-market-share h2 {{
    margin: 0 0 14px;
    font-weight: 500;
  }}

  #download-version-bracket-market-share .download-grid {{
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(260px, 1fr));
    gap: 18px;
  }}

  #download-version-bracket-market-share section {{
    display: grid;
    gap: 10px;
    align-content: start;
  }}

  #download-version-bracket-market-share img {{
    width: 100%;
    max-height: 240px;
    object-fit: contain;
...
<script>
  (() => {{
    const root = document.getElementById("download-version-bracket-market-share");
    const files = {{
      suv: {{
        name: "suv-version-market-share-30-50-2025-2026.png",
        data: "{suv_encoded}"
      }},
      passenger: {{
        name: "passenger-version-market-share-25-50-2025-2026.png",
...
def main():
    ...
    make_png(
        "Vehículos de Pasajeros | Market share por versión",
