...
OUT_JSON = OUT_DIR / "market_results_cb_filtrado.json"
...
def build_segment(df: pd.DataFrame, cfg: dict) -> dict:
    ...
    total_2025 = float(selected["demanda_2025"].sum())
    ...
    def row_payload(row, ranking):
        return {
            ...
            "market_share_2025": float(row["demanda_2025"] / total_2025) if total_2025 else 0,
            "q_mes_2026": clean_int(row["demanda_2026"]),
            "market_share_2026": float(row["demanda_2026"] / total_2026) if total_2026 else 0,
            "q_mes_total": clean_int(row["demanda_total"]),
            "market_share_total": float(row["demanda_total"] / total_market) if total_market else 0,
        }

    rows = [row_payload(row, int(row["rank"])) for _, row in top.iterrows()]
    if len(rest) > 0:
        rest_2025 = float(rest["demanda_2025"].sum())
        rest_2026 = float(rest["demanda_2026"].sum())
        rest_total = float(rest["demanda_total"].sum())
        rows.append(
            {
                "ranking": None,
                "marca": "RESTO",
                ...
                "q_mes_2025": clean_int(rest_2025),
                "market_share_2025": rest_2025 / total_2025 if total_2025 else 0,
                "q_mes_2026": clean_int(rest_2026),
                "market_share_2026": rest_2026 / total_2026 if total_2026 else 0,
                "q_mes_total": clean_int(rest_total),
                "market_share_total": rest_total / total_market if total_market else 0,
            }
        )
    ...
    return {
        ...
        "selected_market_q_mes_2025": clean_int(total_2025),
        "selected_market_q_mes_2026": clean_int(total_2026),
...
def main() -> None:
    ...
    result = {
        ...
        "method": (
            "Rows with excluded CB values are removed first. Version rows are then "
            "aggregated to model-quarter level. Historical model price is a Demanda Anual-weighted "
            "average of quarterly model prices. Price filters are applied to that fixed "
            ...
            "segments": [
                {
                    ...
                    "q_mes_2025": s["selected_market_q_mes_2025"],
                    "q_mes_2026": s["selected_market_q_mes_2026"],
                    "q_mes_total": s["selected_market_q_mes"],
                }
