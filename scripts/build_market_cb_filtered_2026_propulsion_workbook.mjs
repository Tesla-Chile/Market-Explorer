...
const finalPath = path.join(outputDir, "mercado_modelos_2026_filtro_propulsion.xlsx");
const previewPath = path.join(outputDir, "preview_mercado_modelos_2026_filtro_propulsion.png");
...
function combinedPropulsion(rows) {
  const values = [...new Set(rows.map((row) => normalizePropulsion(row.tipo_propulsion)).filter(Boolean))];
  return values.length ? values.join(" / ") : "";
}

function normalizePropulsion(value) {
  if (!value) return "";
  return String(value)
    .replaceAll("H�brido", "Híbrido")
    .replaceAll("Hibrido", "Híbrido")
    .replaceAll("Electrico", "Eléctrico")
    .replaceAll("Enchufable", "Enchufable");
}
...
function adjustedRows(segment) {
  ...
  if (segment.segment === "Vehículo de Pasajeros") {
...
function writeSection(sheet, segment, startRow) {
  ...
  const values = [
    ...
    ...rows.map((row) => [
      ...
      row.marca,
      row.modelo,
      normalizePropulsion(row.tipo_propulsion),
...
const noteRows = [
  ...
  ["Año incluido", String(data.year)],
  ["CB excluidos", data.excluded_cb.join(", ")],
  ["CB incluidos", data.included_cb.map(normalizePropulsion).join(", ")],
  ["Filtro SUV", "$30.000.000 a $50.000.000 CLP sobre precio bruto histórico del modelo."],
  ["Filtro Vehículo de Pasajeros", "$25.000.000 a $50.000.000 CLP sobre precio bruto histórico del modelo."],
  ["Ajustes manuales", "Se combinaron variantes equivalentes y se movieron modelos solicitados a RESTO."],
  ["Tipo de propulsión", "Valor tomado desde la columna CB del archivo fuente. RESTO agrupa varios modelos."],
