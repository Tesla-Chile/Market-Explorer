...
OUTPUTS = {
    "suv": OUT / "suv-brand-market-share-35-50-2025-2026.png",
    "passenger": OUT / "passenger-brand-market-share-30-50-2025-2026.png",
}
PAGE = OUT / "download-brand-bracket-market-share-pngs.html"
...
def compute(path, year, segment, low, high, top_n=8):
    ...
    rows = [
        (row["brand"], int(round(row["units"])), row["units"] / total if total else 0)
...
def draw_panel(draw, y0, period, rows, max_share, chart):
    ...
    text(draw, (label_x, top - 16), "Marca", fill=MUTED, font_obj=SMALL)
    ...
    for idx, (brand, units, share) in enumerate(rows):
        y = top + idx * row_h + 8
        label = brand if len(brand) <= 43 else brand[:42] + "..."
        text(draw, (label_x, y + 5), label, font_obj=LABEL)

        x1 = left
        bar_w = max(3, width * share / max_share)
        fill = REST if brand == "RESTO" else BAR
...
def make_download_page():
    ...
    html = f"""<div id="download-brand-bracket-market-share" class="download-brand-bracket-market-share">
  <h2>Descargar gráficos PNG por marca</h2>
  <div class="download-grid">
    <section>
      <button class="btn btn-primary" type="button" data-file="suv">Descargar PNG SUV</button>
      <img alt="Vista previa SUV por marca" src="data:image/png;base64,{suv_encoded}">
    </section>
    <section>
      <button class="btn btn-primary" type="button" data-file="passenger">Descargar PNG Vehículos de Pasajeros</button>
      <img alt="Vista previa Vehículos de Pasajeros por marca" src="data:image/png;base64,{passenger_encoded}">
    </section>
  </div>
</div>

<style>
  #download-brand-bracket-market-share {{
    width: 100%;
    color: var(--foreground);
  }}

  #download-brand-bracket-market-share h2 {{
    margin: 0 0 14px;
    font-weight: 500;
  }}

  #download-brand-bracket-market-share .download-grid {{
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(260px, 1fr));
    gap: 18px;
  }}

  #download-brand-bracket-market-share section {{
    display: grid;
    gap: 10px;
    align-content: start;
  }}

  #download-brand-bracket-market-share img {{
    width: 100%;
    max-height: 240px;
    object-fit: contain;
...
<script>
  (() => {{
    const root = document.getElementById("download-brand-bracket-market-share");
    const files = {{
      suv: {{
        name: "suv-brand-market-share-35-50-2025-2026.png",
        data: "{suv_encoded}"
      }},
      passenger: {{
        name: "passenger-brand-market-share-30-50-2025-2026.png",
...
def main():
    ...
    suv_2025 = compute(DATA_2025, 2025, "suv", 35_000_000, 50_000_000)
    suv_2026 = compute(DATA_2026, 2026, "suv", 35_000_000, 50_000_000)
    passenger_2025 = compute(DATA_2025, 2025, "passenger", 30_000_000, 50_000_000)
    passenger_2026 = compute(DATA_2026, 2026, "passenger", 30_000_000, 50_000_000)

    make_png(
        "SUV | Market share por marca",
        "Rango precio lista $35MM-$50MM CLP | 2025 completo y Ene-Ago de 2026",
        suv_2025,
        suv_2026,
        OUTPUTS["suv"],
    )
    make_png(
        "Vehículos de Pasajeros | Market share por marca",
