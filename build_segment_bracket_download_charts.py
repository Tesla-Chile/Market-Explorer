...
OUTPUTS = {
    "suv": OUT / "suv-market-share-35-50-2025-2026.png",
    "passenger": OUT / "passenger-market-share-30-50-2025-2026.png",
}
PAGE = OUT / "download-segment-bracket-market-share-pngs.html"
...
def compute(path, year, segment, low, high, top_n=9):
    ...
    grouped["name"] = grouped["brand"] + " " + grouped["model"]
    grouped = grouped.sort_values("units", ascending=False)

    total = grouped["units"].sum()
    top = grouped.head(top_n)
    other_units = total - top["units"].sum()

    rows = [
        (row["name"], int(round(row["units"])), row["units"] / total if total else 0)
        for _, row in top.iterrows()
    ]
    rows.append(("OTROS", int(round(other_units)), other_units / total if total else 0))
    return rows
...
def draw_panel(draw, y0, period, rows, max_share, chart):
    ...
    for idx, (name, units, share) in enumerate(rows):
        y = top + idx * row_h + 8
        label = name if len(name) <= 43 else name[:42] + "..."
...
def make_download_page():
    ...
    html = f"""<div id="download-segment-bracket-market-share" class="download-segment-bracket-market-share">
  <h2>Descargar gráficos PNG</h2>
  <div class="download-grid">
    <section>
      <button class="btn btn-primary" type="button" data-file="suv">Descargar PNG SUV</button>
      <img alt="Vista previa SUV" src="data:image/png;base64,{suv_encoded}">
    </section>
    <section>
      <button class="btn btn-primary" type="button" data-file="passenger">Descargar PNG Vehículos de Pasajeros</button>
      <img alt="Vista previa Vehículos de Pasajeros" src="data:image/png;base64,{passenger_encoded}">
    </section>
  </div>
</div>

<style>
  #download-segment-bracket-market-share {{
    width: 100%;
    color: var(--foreground);
  }}

  #download-segment-bracket-market-share h2 {{
    margin: 0 0 14px;
    font-weight: 500;
  }}

  #download-segment-bracket-market-share .download-grid {{
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(260px, 1fr));
    gap: 18px;
  }}

  #download-segment-bracket-market-share section {{
    display: grid;
    gap: 10px;
    align-content: start;
  }}

  #download-segment-bracket-market-share img {{
    width: 100%;
    max-height: 240px;
    object-fit: contain;
...
<script>
  (() => {{
    const root = document.getElementById("download-segment-bracket-market-share");
    const files = {{
      suv: {{
        name: "suv-market-share-35-50-2025-2026.png",
        data: "{suv_encoded}"
      }},
      passenger: {{
        name: "passenger-market-share-30-50-2025-2026.png",
